...
if (!publishDir) {
  throw new Error(
    `[inject-netlify-identity-widget.js] command must be called with single argument specifying the location of publish dir`
  )
}
...
if (!fs.existsSync(indexHTMLFilePath)) {
  throw new Error(
    `[inject-netlify-identity-widget.js] ${indexHTMLFilePath} was not found`
  )
}
...
function injectHtmlBeforeHtml(html, beforeHtml, injectHtml) {
  ...
  if (index === -1) {
    throw new Error(
      `[inject-netlify-identity-widget.js] ${beforeHtml} was not found in ${indexHTMLFilePath}`
    )
  }
...
indexHTMLData = injectHtmlBeforeHtml(
  indexHTMLData,
  "</head>",
